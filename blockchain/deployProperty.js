...
const deploy = async () => {
  ...
  try {
    const result = await new web3.eth.Contract(compiledPropertyFactory.abi)
      .deploy({ data: '0x' + compiledPropertyFactory.evm.bytecode.object })
      .send({ gas: 8000000, from: accounts[0] });
    console.log("Property Factory Contract deployed to", result.options.address);
  } catch (err) {
    console.log(err);
  }
};
